...
if rol == "admin":
    ...
    if st.session_state.informes == "📝 Informes PDF":
        ...
        # =========================================================
        st.subheader("📍 Distribución Permanente de Mesas")

        # 1. CARGAR DATOS INICIALES
        df_cursos = db_select("cursos")
        df_alumnos = db_select("alumnos")
        # Intentamos cargar la configuración guardada de la tabla 'config_mesas'
        try:
            df_config_prev = db_select("config_mesas")
        except:
            # Si la tabla no existe aún, creamos un DataFrame vacío
            df_config_prev = pd.DataFrame(columns=["id_alumno", "fila"])

        # Unimos alumnos con sus cursos para tener nombres y orden
        df_alumnos_full = df_alumnos.merge(df_cursos, left_on="curso_id", right_on="id", suffixes=("", "_c"))
        # Ordenamos por curso para que la lista de selección sea cómoda (INF -> 6º)
        df_alumnos_full = df_alumnos_full.sort_values(by="curso_id")

        # 2. PREPARAR VALORES POR DEFECTO (Lo que el sistema recuerda)
        def obtener_defaults(fila_num):
            ids_en_fila = df_config_prev[df_config_prev["fila"] == fila_num]["id_alumno"].tolist()
            return df_alumnos_full[df_alumnos_full["id"].isin(ids_en_fila)].to_dict('records')

        col_m1, col_m2, col_m3 = st.columns(3)

        with col_m1:
            m1_sel = st.multiselect("Fila Izquierda", df_alumnos_full.to_dict('records'), 
                                default=obtener_defaults(1), format_func=lambda x: f"{x['nombre']} ({x['nombre_curso']})", key="m1_p")
        with col_m2:
            m2_sel = st.multiselect("Fila Centro", df_alumnos_full.to_dict('records'), 
                                default=obtener_defaults(2), format_func=lambda x: f"{x['nombre']} ({x['nombre_curso']})", key="m2_p")
        with col_m3:
            m3_sel = st.multiselect("Fila Derecha", df_alumnos_full.to_dict('records'), 
                                default=obtener_defaults(3), format_func=lambda x: f"{x['nombre']} ({x['nombre_curso']})", key="m3_p")

        # 3. BOTÓN PARA GUARDAR CAMBIOS (Actualiza la memoria)
        if st.button("💾 Guardar Distribución Actual"):
            try:
                # Borramos la configuración anterior
                supabase.table("config_mesas").delete().neq("fila", 0).execute() # Borra todo
        
                # Preparamos los nuevos datos
                nuevos_registros = []
                for a in m1_sel: nuevos_registros.append({"id_alumno": a["id"], "fila": 1})
                for a in m2_sel: nuevos_registros.append({"id_alumno": a["id"], "fila": 2})
                for a in m3_sel: nuevos_registros.append({"id_alumno": a["id"], "fila": 3})
        
                if nuevos_registros:
                    db_insert("config_mesas", nuevos_registros)
                    st.success("Distribución guardada. Se recordará hasta el próximo cambio.")
                    st.rerun()
            except Exception as e:
                st.error(f"Error al guardar: {e}")

        # 4. GENERAR EL PDF
        if st.button("🖨️ Generar PDF de Mesas"):
            if not any([m1_sel, m2_sel, m3_sel]):
                st.warning("No hay alumnos asignados a las mesas.")
            else:
                buffer = io.BytesIO()
                ...
                c.setFont("Helvetica-Bold", 18)
                c.drawCentredString(page_width/2, page_height - 80, "Distribución de Mesas Comedor")

                # Organizar datos por filas (horizontalmente) para la tabla
                # Importante: Aquí ordenamos cada columna por curso antes de imprimir
                m1_sorted = sorted(m1_sel, key=lambda x: x['curso_id'])
                m2_sorted = sorted(m2_sel, key=lambda x: x['curso_id'])
                m3_sorted = sorted(m3_sel, key=lambda x: x['curso_id'])
        
                max_rows = max(len(m1_sorted), len(m2_sorted), len(m3_sorted))
                tabla_data = [["FILA IZQUIERDA", "FILA CENTRO", "FILA DERECHA"]]
                ...
                for i in range(max_rows):
                    fila_pdf = []
                    for col_idx, lista in enumerate([m1_sorted, m2_sorted, m3_sorted]):
                        if i < len(lista):
                            alumno = lista[i]
                            fila_pdf.append(f"{alumno['nombre']}\n({alumno['nombre_curso']})")
                            # Color según etapa
                            _, color_fondo = obtener_info_etapa(alumno['nombre_curso'])
                            estilos.append(("BACKGROUND", (col_idx, i+1), (col_idx, i+1), color_fondo))
                        else:
                            fila_pdf.append("")
                    tabla_data.append(fila_pdf)

                t = Table(tabla_data, colWidths=[(page_width-100)/3]*3)
                t.setStyle(TableStyle(estilos))
        
                w, h = t.wrap(page_width, page_height)
                t.drawOn(c, 50, (page_height - 120) - h)
        
                c.save()
                st.download_button("Descargar Informe Mesas", buffer.getvalue(), "mesas_comedor.pdf")
