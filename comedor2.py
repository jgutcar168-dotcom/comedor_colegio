...
# ---------------------------------------------------------
    if st.session_state.informes == "📝 Informes PDF":
        ...
        # =========================
        # INFORME DIARIO PARA COCINA (VERSIÓN LIMPIEZA DE TEXTO)
        # =========================
        ...
        if st.button("Generar PDF Diario", type="primary"):
            # 1. Diccionario de Cursos (ID -> NOMBRE)
            # Forzamos que las llaves sean strings para comparar fácilmente
            dict_nombres_curso = {str(k): str(v) for k, v in zip(df_cursos["id"], df_cursos["nombre"])}
            
            # 2. Diccionario de Alumnos para nombres y sus cursos asignados
            dict_nombres_alu = {str(k): str(v) for k, v in zip(df_alumnos["id"], df_alumnos["nombre"])}
            dict_curso_id_alu = {str(k): str(v) for k, v in zip(df_alumnos["id"], df_alumnos["curso_id"])}

            # 3. Filtramos la asistencia de hoy
            df_dia = df_asistencia[df_asistencia["fecha"] == fecha_diario_str].copy()
            comen = df_dia[df_dia["asiste"] == True].copy()

            if comen.empty:
                st.warning(f"No hay comensales registrados para el día {fecha_diario.strftime('%d/%m/%Y')}")
            else:
                datos_procesados = []
                for _, fila in comen.iterrows():
                    alu_id = str(fila["alumno_id"])
                    
                    # Buscamos qué curso tiene el alumno
                    val_curso = dict_curso_id_alu.get(alu_id, "Sin Curso")
                    
                    # LÓGICA DE LIMPIEZA:
                    # Si 'val_curso' ya es un nombre (como '5º B'), lo usamos directamente.
                    # Si 'val_curso' es un ID numérico, buscamos su nombre real en dict_nombres_curso.
                    nombre_final_curso = dict_nombres_curso.get(val_curso, val_curso)
                    
                    datos_procesados.append({"nombre_curso": nombre_final_curso})

                df_resumen = pd.DataFrame(datos_procesados)
                ...
                # Agrupar y contar
                conteo = df_resumen.groupby("nombre_curso").size().reset_index(name="Cantidad")
                conteo = conteo.sort_values("nombre_curso")

                tabla_data = [["Curso", "Comensales"]]
                for _, row in conteo.iterrows():
                    # Evitamos que aparezca la palabra 'None' o vacíos
                    nombre_c = str(row["nombre_curso"]) if row["nombre_curso"] else "Sin Curso"
                    tabla_data.append([nombre_c, row["Cantidad"]])

                tabla_data.append(["TOTAL GENERAL", conteo["Cantidad"].sum()])
                ...
                c.drawString(50, y_actual, "Observaciones / Motivos:")
                y_actual -= 20
                
                obs_data = df_dia[df_dia["motivo"].fillna("") != ""]
                
                c.setFont("Helvetica", 10)
                if obs_data.empty:
                    c.drawString(60, y_actual, "No hay observaciones para hoy.")
                else:
                    for _, row in obs_data.iterrows():
                        nombre_alu = dict_nombres_alu.get(str(row["alumno_id"]), "Desconocido")
                        if y_actual < 80:
                            add_page_number(c)
                            c.showPage()
                            y_actual = 800
                        text = f"• {nombre_alu}: {row['motivo']}"
