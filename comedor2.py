...
if diario == "📋 Pasar lista":
   ...
   # ... (código anterior igual hasta el inicio del bucle for)
    ...
    for _, alumno in alumnos_curso.iterrows():
        ...
        # ELIMINADO: Ya no pedimos el motivo si el checkbox es False
        motivos[alumno["id"]] = "" 
    ...
    if st.button("Guardar asistencia"):
        ...
        for alumno_id in estado.keys():
            registros.append({
                ...
                "motivo": "" # Guardamos siempre vacío para mantener la estructura
            })
    ...
    # ---------------------------------------------------------
        ...
        with st.form("nuevo_profesor"):
            ...
            if st.form_submit_button("Guardar"):
                ...
                if duplicado:
                    st.error("Este profesor ya está registrado (coincide aunque cambien tildes o mayúsculas).")
                else:
                    try:
                        ...
                        st.success("Profesor añadido correctamente.")
                        st.rerun()

                    except Exception as e:
                        st.error("Ha ocurrido un error al registrar el profesor.")
        ...
        if not df_profes.empty:
            ...
            if st.button("Actualizar nombre"):
                ...
                if nuevo_nombre.strip() == "":
                    ...
                    nuevo_norm = normalizar(nuevo_nombre)

                    # Comprobar duplicados excepto el propio profesor
                    duplicado = any(
                        normalizar(row["usuario"]) == nuevo_norm and row["id"] != prof_sel["id"]
                        for _, row in df_profes.iterrows()
                    )

                    if duplicado:
                        st.error("Ya existe otro profesor con ese nombre (coincide aunque cambien tildes o mayúsculas).")
                    else:
                        try:
            ...
            index_curso = indices[0] if indices else 0
