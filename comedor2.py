...
# ---------------------------------------------------------
    ...
    # Unimos asistencia con alumnos para saber quién es de comedor
    resumen = df_hoy.merge(df_alumnos, left_on="alumno_id", right_on="id")

    # --- FILTRO CRÍTICO --- 
    # Solo contamos para cocina si: Asiste HOY Y NO es 'solo_pt_al'
    # Usamos != True para incluir False y NULLs antiguos
    comen_real = resumen[(resumen["asiste"] == True) & (resumen["solo_pt_al"] != True)]

    if comen_real.empty:
        st.info("Hay asistencia registrada, pero ningún alumno de los presentes es de comedor (posiblemente todos sean Solo PT/AL).")
    
    # Arreglo de nombres de columnas tras el merge
    col_curso = "curso_id_y" if "curso_id_y" in resumen.columns else "curso_id_x" if "curso_id_x" in resumen.columns else "curso_id"
    comen_real = comen_real.rename(columns={col_curso: "curso_id_final"})

    # Unir con cursos para sacar los nombres
...
# ---------------------------------------------------------
    ...
    # Filtramos por fecha (independientemente de si asiste o no, para saber si el profe cumplió)
    df_hoy = df_asistencia[df_asistencia["fecha"] == fecha_hoy]

    # Obtenemos los IDs de los cursos que ya enviaron datos hoy
