...
if diario == "📋 Pasar lista":
    ...
    if not cursos_disponibles:
        ...
        # --- GUARDAR ASISTENCIA ---
        # --- GUARDAR ASISTENCIA (VERSIÓN DIRECTA Y SEGURA) ---
        if st.button("Guardar asistencia", type="primary", use_container_width=True):
            ...
            if registros:
                try:
                    # Guardado directo a Supabase sin intermediarios
                    res = supabase.table("asistencia").upsert(
                        registros, 
                        on_conflict="alumno_id,fecha"
                    ).execute()
                    
                    # Verificamos si realmente se escribió algo
                    if res.data:
                        st.success(f"✅ Grabado: {len(res.data)} alumnos registrados en la base de datos.")
                        st.cache_data.clear() # Forzamos a que el admin lea datos nuevos
                        st.balloons()
                    else:
                        st.error("⚠️ El servidor no devolvió datos. Revisa la tabla en Supabase.")
                except Exception as e:
                    st.error("❌ ERROR REAL DE BASE DE DATOS:")
                    st.code(str(e)) # Si sale un error aquí, cópialo y dímelo
            else:
...
# ---------------------------------------------------------
    ...
    st.cache_data.clear() # Limpiar memoria para ver lo recién guardado
    ...
    # Filtramos asistencia de hoy y que asistan
    df_hoy = df_asistencia[(df_asistencia["fecha"] == fecha_hoy) & (df_asistencia["asiste"] == True)]

    if df_hoy.empty:
        st.warning("No hay registros de asistencia positiva para hoy.")
    else:
        # Aseguramos tipos numéricos para el cruce
        df_hoy["alumno_id"] = df_hoy["alumno_id"].astype(int)
        df_alumnos["id"] = df_alumnos["id"].astype(int)
        
        # Cruce Asistencia + Alumnos
        resumen = df_hoy.merge(df_alumnos, left_on="alumno_id", right_on="id")
        
        # Cruce con Cursos (usamos el curso_id que viene de la tabla alumnos)
        df_cursos["id"] = df_cursos["id"].astype(int)
        resumen = resumen.merge(df_cursos, left_on="curso_id_y", right_on="id")

        st.metric("TOTAL COMENSALES", len(resumen))
        
        st.write("### Desglose por Curso")
