...
if diario == "📋 Pasar lista":
    ...
    # Intentamos forzar la lectura fresca de Supabase
    df_alumnos = db_select("alumnos")
    df_cursos = db_select("cursos")
    df_asistencia = db_select("asistencia")

    # Si por algún motivo la tabla viene vacía, creamos un DataFrame de emergencia
    # para que la app no se rompa con el KeyError ['fecha']
    ...
    if "fecha" not in df_asistencia.columns:
        ...
        st.info("Si la lista anterior está vacía, reinicia la app en el menú de Streamlit (Settings -> Reboot).")
        st.stop()
    ...
    if not cursos_disponibles:
        ...
        if st.button("💾 Guardar Asistencia", type="primary", use_container_width=True):
            ...
            if registros:
                # DEBUG: Ver los datos que vamos a enviar
                # st.write("Enviando a Supabase:", registros) 
                
                try:
                    # Agregamos on_conflict para que sepa que si coinciden alumno y fecha, debe ACTUALIZAR
                    ...
                    if res.data:
                        st.success(f"✅ ¡CONSEGUIDO! {len(res.data)} registros grabados/actualizados.")
                        st.cache_data.clear()
                        st.balloons()
                    else:
                        st.error("❌ El servidor aceptó la orden pero no guardó nada.")
                except Exception as e:
...
# ---------------------------------------------------------
    ...
    # Usamos una lógica segura por si la tabla está vacía
    ...
    # Nos aseguramos de que curso_id e id de cursos sean del mismo tipo
    df_cursos["id"] = df_cursos["id"].astype(int)
    
    # Cursos que han pasado lista hoy
    df_ok = df_cursos[df_cursos["id"].isin(cursos_con_lista)]
    
    # Cursos que NO están en la lista de hoy
    ...
    if cursos_faltan.empty:
        st.success("¡Excelente! Todos los cursos han pasado lista hoy.")
    else:
