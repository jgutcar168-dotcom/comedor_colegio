...
if rol == "admin":
    ...
    if st.session_state.informes == "📝 Informes PDF":
       ...
       # =========================
        ...
        if isinstance(rango_m, list) or isinstance(rango_m, tuple):
            if len(rango_m) == 2:
                ...
                if st.button("Generar Informe por Rango"):
                    ...
                    if not profes_comen.empty:
                        # Cabecera: Nombre + día del mes + Total
                        ...
                        for _, prof in profes_comen.iterrows():
                            ...
                            for d in dias_rango:
                                ...
                                asiste = not comidas_rango[(comidas_rango["maestro_id"] == prof["id"]) & (comidas_rango["fecha"] == f_str)].empty
                                fila.append("X" if asiste else "")
                                if asiste: total += 1
                            fila.append(total)
                            data_c.append(fila)

                        # Ajustamos el ancho de celdas según el número de días
                    ...
                    if not profes_agua.empty:
                        ...
                        for _, prof in profes_agua.iterrows():
                            ...
                            for d in dias_rango:
                                f_str = d.strftime("%Y-%m-%d")
                                reg = agua_rango[(agua_rango["maestro_id"] == prof["id"]) & (agua_rango["fecha"] == f_str)]
                                if not reg.empty:
