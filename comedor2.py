...
# ---------------------------------------------------------
# PASAR LISTA (VERSIÓN RESET-TOTAL)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    if rol == "cocina":
        st.info("💡 Tu perfil está configurado para el Panel de Cocina.")
    else:
        # 1. Limpieza inicial para asegurar datos frescos
        st.cache_data.clear() 
        ...
        if not cursos_disponibles:
            ...
            curso_sel = st.selectbox(
                "Selecciona el curso para pasar lista:",
                cursos_disponibles,
                format_func=lambda x: x["nombre"],
                key="selector_curso_main"
            )
            
            c_id = int(curso_sel["id"])

            # 3. FILTRADO Y CARGA DE ESTADO REAL
            alumnos_curso = df_alumnos[df_alumnos["curso_id"] == c_id].copy().sort_values(by="nombre")
            
            # Buscamos qué hay grabado HOY para este curso
            asist_hoy = df_asistencia[(df_asistencia["fecha"] == fecha_hoy) & (df_asistencia["curso_id"] == c_id)]

            st.subheader(f"Lista de {curso_sel['nombre']}")
            st.write("Estado actual en la base de datos:")

            # 4. FORMULARIO DINÁMICO
            # Usamos un diccionario para capturar los nuevos valores
            nuevos_estados = {}
            
            for _, alu in alumnos_curso.iterrows():
                # Buscamos si ya existe registro para este alumno concreto
                ya_grabado = asist_hoy[asist_hoy["alumno_id"] == alu["id"]]
                
                # Si no hay registro, por defecto asiste (True). Si hay, lo que diga la DB.
                valor_db = True if ya_grabado.empty else bool(ya_grabado["asiste"].iloc[0])

                # CLAVE CRÍTICA: La key incluye el ID del alumno Y el ID del curso
                # Esto obliga a Streamlit a no mezclar datos entre cursos
                check = st.checkbox(
                    alu["nombre"],
                    value=valor_db,
                    key=f"check_final_{alu['id']}_{c_id}" 
                )
                nuevos_estados[alu["id"]] = check

            # 5. BOTÓN DE GUARDADO SEGURO
            if st.button("💾 CONFIRMAR Y GUARDAR ASISTENCIA", type="primary", use_container_width=True):
                lista_upsert = []
                for a_id, asiste_bool in nuevos_estados.items():
                    lista_upsert.append({
                        "fecha": fecha_hoy,
                        "alumno_id": int(a_id),
                        "curso_id": int(c_id),
                        "asiste": bool(asiste_bool),
                        "motivo": ""
                    })

                try:
                    # Ejecutamos el Upsert
                    res = supabase.table("asistencia").upsert(lista_upsert, on_conflict="alumno_id,fecha").execute()
                    
                    if res.data:
                        st.success(f"✅ ¡Guardado con éxito! {len(res.data)} registros actualizados.")
                        # LIMPIEZA ABSOLUTA DE CACHÉ TRAS GUARDAR
                        st.cache_data.clear()
                        if "df_asistencia" in st.session_state:
                            del st.session_state["df_asistencia"]
                        
                        st.balloons()
                        # El RERUN es vital para que al volver a dibujar lea los nuevos valores
                        st.rerun()
                except Exception as e:
                    st.error(f"Error crítico al guardar: {e}")
                    
# ---------------------------------------------------------
# PANEL DE COCINA (VERSIÓN FINAL)
# ---------------------------------------------------------
    ...
    fecha_hoy = datetime.now().strftime("%Y-%m-%d")
    st.info(f"Mostrando datos para hoy: **{datetime.now().strftime('%d/%m/%Y')}**")

    # 1. Filtrar registros de hoy donde asisten = True
    df_hoy = df_asistencia[(df_asistencia["fecha"] == fecha_hoy) & (df_asistencia["asiste"] == True)].copy()

    if df_hoy.empty:
        st.warning("⚠️ Todavía no hay asistencias confirmadas para hoy.")
        st.stop()

    try:
        # 2. Aseguramos tipos para el cruce
        df_hoy["curso_id"] = df_hoy["curso_id"].astype(int)
        df_cursos["id"] = df_cursos["id"].astype(int)

        # 3. Unimos con la tabla de cursos para tener los nombres
        resumen = df_hoy.merge(df_cursos, left_on="curso_id", right_on="id")

        if resumen.empty:
            st.error("❌ Error: No se encuentran los nombres de los cursos en la base de datos.")
        else:
            # MÉTRICA PRINCIPAL
            st.metric("TOTAL COMENSALES HOY", len(resumen))
            
            # TABLA RESUMEN POR CURSO
            conteo = resumen.groupby("nombre").size().reset_index(name="Alumnos")
            conteo = conteo.sort_values("nombre") # Orden alfabético de cursos
            
            st.subheader("Desglose por grupos")
            st.table(conteo.rename(columns={"nombre": "Curso", "Alumnos": "Nº Comensales"}))
            
    except Exception as e:
        st.error(f"Hubo un problema al procesar los datos: {e}")

# ---------------------------------------------------------
# CONTROL DE ASISTENCIA (VERSIÓN FINAL)
# ---------------------------------------------------------
    ...
    # Cursos que han pasado lista hoy (independientemente de si asisten o no)
    if not df_asistencia.empty and "fecha" in df_asistencia.columns:
        cursos_con_lista = df_asistencia[df_asistencia["fecha"] == fecha_hoy]["curso_id"].unique().astype(int).tolist()
    else:
        cursos_con_lista = []
