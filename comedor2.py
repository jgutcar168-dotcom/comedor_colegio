...
# ---------------------------------------------------------
    ...
    st.cache_data.clear()
    st.header("🕵️ Diagnóstico de Datos")

    df_asistencia = db_select("asistencia")
    df_cursos = db_select("cursos")
    
    fecha_app = datetime.now().strftime("%Y-%m-%d")
    
    st.write(f"📅 Fecha que busca la App hoy: **{fecha_app}**")

    if df_asistencia.empty:
        st.error("❌ La tabla 'asistencia' ha llegado vacía a la App.")
    else:
        # --- PASO A PASO ---
        # 1. ¿Qué fechas hay en la base de datos?
        fechas_reales = df_asistencia["fecha"].unique().tolist()
        st.write(f"📅 Fechas encontradas en la DB: {fechas_reales}")

        # 2. Filtrar por fecha
        df_hoy = df_asistencia[df_asistencia["fecha"] == fecha_app].copy()
        st.write(f"📊 Registros encontrados para hoy: {len(df_hoy)}")

        if len(df_hoy) > 0:
            # 3. Ver qué cursos hay en esos registros
            ids_en_asistencia = df_hoy["curso_id"].unique().tolist()
            ids_en_cursos = df_cursos["id"].unique().tolist()
            st.write(f"🆔 IDs de curso en Asistencia: {ids_en_asistencia}")
            st.write(f"🆔 IDs de curso en tabla Cursos: {ids_en_cursos}")
            ...
            try:
                ...
                # Unión
                resumen = df_hoy.merge(df_cursos, left_on="curso_id", right_on="id")
                
                if resumen.empty:
                    st.warning("⚠️ El MERGE ha vaciado los datos. Los IDs de curso no coinciden entre tablas.")
                else:
                    st.success(f"✅ Se han cruzado {len(resumen)} registros correctamente.")
                    conteo = resumen.groupby("nombre").size().reset_index(name="Alumnos")
                    st.table(conteo)
            except Exception as e:
                st.error(f"Error técnico: {e}")
        else:
            st.warning("No hay datos para la fecha de hoy. Intenta pasar lista de nuevo en un curso.")
# ---------------------------------------------------------
...
# ---------------------------------------------------------
    ...
    df_asistencia = db_select("asistencia")
    df_cursos_raw = db_select("cursos")
    fecha_hoy = datetime.now().strftime("%Y-%m-%d")
    ...
    # 1. Obtenemos los cursos que han pasado lista hoy
    if not df_asistencia.empty and "fecha" in df_asistencia.columns:
        df_hoy = df_asistencia[df_asistencia["fecha"] == fecha_hoy]
        # IMPORTANTE: Convertimos a lista de enteros para comparar bien
        cursos_con_lista = df_hoy["curso_id"].unique().astype(int).tolist()
    else:
        cursos_con_lista = []

    # 2. Cursos OK y Faltan
