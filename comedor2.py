...
# ---------------------------------------------------------
# PASAR LISTA (VERSIÓN CORREGIDA Y BLINDADA)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    if rol == "cocina":
        st.info("💡 Tu perfil está configurado para consultar el **Panel de Cocina**.")
    else:
        st.cache_data.clear() 
        ...
        if rol == "admin":
            ...
            msg_sel = "Selecciona curso (Admin)"
        else:
            ...
            msg_sel = "Selecciona curso asignado"

        if not cursos_disponibles:
            st.warning("No tienes cursos asignados.")
        else:
            # IMPORTANTE: El selector define el curso_id_actual
            curso_sel = st.selectbox(
                msg_sel,
                cursos_disponibles,
                format_func=lambda x: x["nombre"],
                key="selector_lista_definitivo"
            )
            
            # ESTA VARIABLE ES LA CLAVE
            curso_id_actual = int(curso_sel["id"])

            # Filtramos alumnos de ESTE curso y ordenamos
            alumnos_curso = df_alumnos[df_alumnos["curso_id"] == curso_id_actual].copy()
            alumnos_curso = alumnos_curso.sort_values(by="nombre") 

            st.subheader(f"Curso: {curso_sel['nombre']}")

            # Consultamos si ya hay registros hoy para este curso exacto
            asistencia_hoy = df_asistencia[
                (df_asistencia["fecha"] == fecha_hoy) & 
                (df_asistencia["curso_id"] == curso_id_actual)
            ]

            # Creamos el formulario de checkboxes
            estado = {}
            st.write("Selecciona los alumnos que asisten:")
            
            for _, alumno in alumnos_curso.iterrows():
                # Buscamos si este alumno concreto ya tiene marca hoy
                reg_alumno = asistencia_hoy[asistencia_hoy["alumno_id"] == alumno["id"]]
                valor_defecto = True if reg_alumno.empty else bool(reg_alumno["asiste"].iloc[0])

                check = st.checkbox(
                    alumno["nombre"],
                    value=valor_defecto,
                    key=f"chk_{alumno['id']}_{curso_id_actual}" # Key única por curso/alumno
                )
                estado[alumno["id"]] = check
            ...
            if st.button("💾 Guardar Asistencia", type="primary", use_container_width=True):
                ...
                for alu_id, asiste_val in estado.items():
                    registros_a_enviar.append({
                        "fecha": fecha_hoy,
                        "alumno_id": int(alu_id),
                        "curso_id": curso_id_actual, # Usamos la variable blindada
                        "asiste": bool(asiste_val),
                        "motivo": ""
                    })

                if registros_a_enviar:
                    try:
                        # Enviamos a Supabase
                        res = supabase.table("asistencia").upsert(
                            registros_a_enviar, 
                            on_conflict="alumno_id,fecha"
                        ).execute()
                        
                        if res.data:
                            st.success(f"✅ ¡Guardado! {len(res.data)} alumnos registrados en {curso_sel['nombre']}.")
                            st.cache_data.clear()
                            st.balloons()
                            # Forzamos recarga para que el Panel de Cocina se entere
                            st.rerun()
                    except Exception as e:
