...
# ---------------------------------------------------------
# MENÚ LATERAL REFORMULADO (EVITA ERRORES DE SESIÓN)
# ---------------------------------------------------------
...
with st.sidebar:
    ...
    with st.expander("📅 Diario", expanded=True):
        ...
        # 1. Definimos las opciones permitidas estrictamente por rol
        if rol == "admin":
        ...
        # 2. Calculamos el índice por defecto para evitar que cargue "Pasar lista" si no debe
        # Si la opción actual no es válida para el rol, ponemos la primera de su lista
        actual = st.session_state.get("diario")
        if actual not in opciones_diario:
            indice_defecto = 0
        else:
            indice_defecto = opciones_diario.index(actual)

        # 3. El componente Radio sin funciones extra que den error
        diario = st.radio(
            " ",
            opciones_diario,
