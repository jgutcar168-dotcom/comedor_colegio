            ...
            if registros:
                try:
                    ...
                    if res.data:
                        ...
                        # --- LAS LÍNEAS MÁGICAS PARA ACTUALIZAR TODO ---
                        st.cache_data.clear()  # Limpia la memoria de funciones
                        if "df_asistencia" in st.session_state:
                            del st.session_state["df_asistencia"] # Borra la tabla guardada en sesión
                    
                        st.balloons()
                        st.rerun() # Fuerza a la app a recargarse por completo
                    else:
        ...
        with c_btn2:
            ...
            if st.button("🖨️ Generar PDF de Situación", type="primary", use_container_width=True):
                ...
                if not any([m1_sel, m2_sel, m3_sel]):
                    ...
                    # --- CONFIGURACIÓN DE ALERTAS ALÉRGICAS ---
                    alertas_gluten = ["Ceballos Ruíz, Lucía", "García Ruíz, Lucía"]
                    color_alerta = colors.orange  # Puedes usar colors.lightcoral para un rojo suave
                    ...
                    for i in range(max_filas):
                        ...
                        for col_idx, lista in enumerate([m1_pdf, m2_pdf, m3_pdf]):
                            if i < len(lista):
                                alu = lista[i]
                                nombre_completo = alu['nombre']
                                
                                # 1. Determinamos el color base por su etapa
                                _, color_f = obtener_info_etapa(alu['nombre_curso'])
                                
                                # 2. MEJORA: Comprobamos si es una de las niñas con alergia
                                if nombre_completo in alertas_gluten:
                                    color_f = color_alerta
                                    # Opcional: Añadimos un aviso visual al texto
                                    texto_celda = f"{nombre_completo}\n({alu['nombre_curso']})"
                                else:
                                    texto_celda = f"{nombre_completo}\n({alu['nombre_curso']})"
                                
                                fila_cont.append(texto_celda)
                                estilos_tabla.append(("BACKGROUND", (col_idx, i+1), (col_idx, i+1), color_f))
                            else:
                                fila_cont.append("")
                        tabla_data.append(fila_cont)
