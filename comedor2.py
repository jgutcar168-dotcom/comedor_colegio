...
with st.sidebar:
    ...
    # ---------------------------
    # GRUPO: DIARIO (Sustituye esta parte en tu Sidebar)
    # ---------------------------
    with st.expander("📅 Diario", expanded=True):
        ...
        # 2. ¡ESTA ES LA CLAVE!: Si el usuario es cocina y la opción actual no es válida, 
        # forzamos que la selección sea el Panel de Cocina.
        if rol == "cocina" and st.session_state.get("diario") not in opciones_diario:
            st.session_state["diario"] = "🍽️ Panel cocina"

        # 3. Creamos el radio button
        diario = st.radio(
            " ",
            opciones_diario,
