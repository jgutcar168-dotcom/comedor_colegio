...
# ---------------------------------------------------------
    ...
    # 1. Filtrar registros de hoy que SI ASISTEN
    ...
    if df_hoy.empty:
        ...
        try:
            # 2. Forzar tipos para asegurar el cruce
            df_hoy["alumno_id"] = df_hoy["alumno_id"].astype(int)
            df_alumnos["id"] = df_alumnos["id"].astype(int)
            df_cursos["id"] = df_cursos["id"].astype(int)

            # 3. Unión limpia: Usamos el curso_id que viene directamente en la tabla ASISTENCIA
            # Esto es mucho más seguro que sacarlo del alumno
            resumen = df_hoy.merge(df_cursos, left_on="curso_id", right_on="id")

            st.metric("TOTAL COMENSALES", len(resumen))
            
            # 4. Agrupar por el nombre del curso que viene de df_cursos
            conteo = resumen.groupby("nombre").size().reset_index(name="Alumnos")
            
            # Ordenar alfabéticamente los cursos para que sea legible
            conteo = conteo.sort_values("nombre")
