...
# ---------------------------------------------------------
    if st.session_state.informes == "📝 Informes PDF":
        ...
        # =========================
        # INFORME DIARIO PARA COCINA (VERSIÓN ANTIFALLOS)
        # =========================
        ...
        if st.button("Generar PDF Diario", type="primary"):
            # 1. Limpieza y Normalización de tipos
            df_asistencia["alumno_id"] = pd.to_numeric(df_asistencia["alumno_id"], errors='coerce')
            df_alumnos["id"] = pd.to_numeric(df_alumnos["id"], errors='coerce')
            df_alumnos["curso_id"] = pd.to_numeric(df_alumnos["curso_id"], errors='coerce')
            df_cursos["id"] = pd.to_numeric(df_cursos["id"], errors='coerce')
            ...
            if comen.empty:
                ...
                # --- PASO CRÍTICO: UNIÓN DE TABLAS ---
                # Unimos asistencia con alumnos (traemos solo lo necesario para evitar duplicados)
                # Usamos 'alumno_id' de asistencia e 'id' de alumnos
                resumen = comen.merge(
                    df_alumnos[['id', 'nombre', 'curso_id']], 
                    left_on="alumno_id", 
                    right_on="id", 
                    how="left"
                )

                # Unimos con cursos usando el 'curso_id' que acabamos de traer del alumno
                ...
                # Renombramos la columna del nombre del curso para que el código sea claro
                resumen = resumen.rename(columns={"nombre_y": "nombre_curso", "nombre_x": "nombre_alumno"})
                
                # Si por algún motivo 'nombre_curso' no existe, lo creamos de emergencia
                if "nombre_curso" not in resumen.columns:
                    resumen["nombre_curso"] = "Sin Curso"
                ...
                obs = df_dia[df_dia["motivo"].fillna("") != ""].merge(df_alumnos[['id', 'nombre']], left_on="alumno_id", right_on="id")
