...
# ---------------------------------------------------------
    if st.session_state.informes == "📝 Informes PDF":
        ...
        if st.button("Generar PDF por Curso", type="primary"):
            # Limpieza de tipos y preparación de datos
            dict_alu_curso = {str(k): str(v) for k, v in zip(df_alumnos["id"], df_alumnos["curso_id"])}
            ...
            if curso_sel == "Todos los cursos":
                ...
                cursos_a_procesar = df_cursos[df_cursos["nombre"] == curso_sel].to_dict(orient="records")

            # Filtrar asistencia del día
            df_dia = df_asistencia[df_asistencia["fecha"] == fecha_curso_str].copy()
            ...
            for curso in cursos_a_procesar: 
                ...
                # Filtrar alumnos que pertenecen a este curso usando el diccionario
                tabla_data = [["Alumno", "Asiste"]]
                
                # Buscamos en la asistencia quiénes pertenecen a este curso
                filas_curso = []
                ...
                filas_curso.sort(key=lambda x: x[0])
                tabla_data.extend(filas_curso if filas_curso else [["No hay datos registrados", "-"]])

                # Configuración de tabla centrada
                anchos = [300, 100]
            ...
            st.download_button(
                ...
                data=buffer.getvalue(),
                file_name=f"curso_{curso_sel}_{fecha_curso_str}.pdf",
                mime="application/pdf"
            )

    
        # =========================
        # INFORME MENSUAL (VERSIÓN SIN ERRORES DE COLUMNAS)
        # =========================
        ...
        if st.button("Generar PDF Mensual", type="primary"):
            # 1. Preparar mapeos
            dict_alu_curso = {str(k): str(v) for k, v in zip(df_alumnos["id"], df_alumnos["curso_id"])}
            df_cur_filt = df_cursos[df_cursos["nombre"].str.lower() != "ninguno"].copy()
            
            dias_mes = calendar.monthrange(año, mes)[1]
            
            # 2. Obtener TODA la asistencia del mes de una sola vez (más eficiente)
            fecha_inicio = f"{año}-{mes:02d}-01"
            fecha_fin = f"{año}-{mes:02d}-{dias_mes:02d}"
            asis_mes = df_asistencia[(df_asistencia["fecha"] >= fecha_inicio) & 
                                    (df_asistencia["fecha"] <= fecha_fin) & 
                                    (df_asistencia["asiste"] == True)].copy()
            ...
            for _, curso in df_cur_filt.iterrows():
                ...
                for d in range(1, dias_mes+1):
                    ...
                    # Contamos cuántos alumnos de este curso asistieron este día
                    # Usamos el diccionario para saber de qué curso es cada alumno en asis_mes
                    asis_dia = asis_mes[asis_mes["fecha"] == f_busqueda]
                    count = 0
            ...
            c.setFont("Helvetica-Bold", 18)
            c.drawCentredString(page_width/2, page_height - 110, f"RECUENTO MENSUAL - {mes}/{año}")

            # Ajustar tamaño de fuente según días del mes para que quepa
            fontSize = 9 if dias_mes > 28 else 10
            
            tabla = Table(tabla_data, colWidths=[80] + [20]*dias_mes + [45])
            estilo = [
                ("BACKGROUND", (0,0), (-1,0), colors.lightgrey),
            ...
            c.save()
            st.download_button(
                label="📥 Descargar Recuento Mensual",
                data=buffer.getvalue(),
                file_name=f"mensual_{mes}_{año}.pdf",
                mime="application/pdf"
            )
