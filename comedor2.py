...
# ---------------------------------------------------------
    if st.session_state.informes == "📝 Informes PDF":
        ...
        # =========================
        # INFORME DIARIO PARA COCINA (VERSIÓN DEFINITIVA SIN MERGE)
        # =========================
        ...
        if st.button("Generar PDF Diario", type="primary"):
            # 1. Normalización de tipos para evitar fallos de comparación
            df_asistencia["alumno_id"] = pd.to_numeric(df_asistencia["alumno_id"], errors='coerce')
            df_alumnos["id"] = pd.to_numeric(df_alumnos["id"], errors='coerce')
            df_alumnos["curso_id"] = pd.to_numeric(df_alumnos["curso_id"], errors='coerce')
            df_cursos["id"] = pd.to_numeric(df_cursos["id"], errors='coerce')

            # 2. Creamos "diccionarios" para buscar nombres rápido sin hacer Merges
            # Diccionario: ID_ALUMNO -> NOMBRE_ALUMNO
            dict_nombres_alu = dict(zip(df_alumnos["id"], df_alumnos["nombre"]))
            # Diccionario: ID_ALUMNO -> ID_CURSO
            dict_curso_id_alu = dict(zip(df_alumnos["id"], df_alumnos["curso_id"]))
            # Diccionario: ID_CURSO -> NOMBRE_CURSO
            dict_nombres_curso = dict(zip(df_cursos["id"], df_cursos["nombre"]))
            ...
            if comen.empty:
                ...
                # 4. Construimos la tabla de datos manualmente (Evita errores de columnas de Pandas)
                datos_procesados = []
                for _, fila in comen.iterrows():
                    alu_id = fila["alumno_id"]
                    c_id = dict_curso_id_alu.get(alu_id)
                    nombre_c = dict_nombres_curso.get(c_id, "Sin Curso")
                    datos_procesados.append({"nombre_curso": nombre_c})
                ...
                for _, row in conteo.iterrows():
                    tabla_data.append([str(row["nombre_curso"]), row["Cantidad"]])
                ...
                if obs_data.empty:
                    ...
                    for _, row in obs_data.iterrows():
                        nombre_alu = dict_nombres_alu.get(row["alumno_id"], "Desconocido")
                        if y_actual < 80:
                            add_page_number(c)
                            c.showPage()
                            y_actual = 800
