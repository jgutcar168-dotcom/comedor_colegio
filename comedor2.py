...
if diario == "📋 Pasar lista":
    ...
    if not cursos_disponibles:
        ...
        # --- GUARDAR ASISTENCIA ---
        # --- GUARDAR ASISTENCIA (VERSIÓN REFORZADA) ---
        if st.button("Guardar asistencia", type="primary"):
            registros = []
            for alu_id, asiste_val in estado.items():
                registros.append({
                    "fecha": fecha_hoy,
                    "alumno_id": int(alu_id),  # Forzamos número
                    "curso_id": int(curso_id),  # Forzamos número
            ...
            if registros:
                try:
                    # Usamos directamente la conexión para evitar intermediarios
                    supabase.table("asistencia").upsert(
                        registros, 
                        on_conflict="alumno_id,fecha"
                    ).execute()
                    
                    st.cache_data.clear() # Limpiamos memoria para que el Admin vea los cambios
                    st.success(f"✅ Asistencia de {curso_sel['nombre']} guardada en la base de datos")
                    st.balloons()
                    # No hacemos rerun inmediatamente para asegurar que la conexión termine
                except Exception as e:
