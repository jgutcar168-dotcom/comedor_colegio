...
# ---------------------------------------------------------
# MENÚ LATERAL PROTEGIDO Y DINÁMICO
# ---------------------------------------------------------
prof = st.session_state.profesor
rol = prof["rol"]

# --- LIMPIEZA DINÁMICA DE SESIÓN SEGÚN ROL ---
# Si un usuario de cocina entra y tiene guardado "Pasar lista", lo borramos
if rol == "cocina" and st.session_state.get("diario") == "📋 Pasar lista":
    st.session_state["diario"] = "🍽️ Panel cocina"
elif rol == "profe" and st.session_state.get("diario") in ["🍽️ Panel cocina", "✔️ Control de asistencia"]:
    st.session_state["diario"] = "📋 Pasar lista"

with st.sidebar:
    st.markdown("""
        <style>
            .sidebar-title { font-size: 22px; font-weight: bold; margin-bottom: -10px; }
        </style>
    ...
    with st.expander("📅 Diario", expanded=True):
        ...
        # Definición estricta de opciones
        if rol == "admin":
            opciones_diario = ["📋 Pasar lista", "🍽️ Panel cocina", "✔️ Control de asistencia", "📊 Situación en mesa"]
        elif rol == "cocina":
            opciones_diario = ["🍽️ Panel cocina", "📊 Situación en mesa"]
        else:
            opciones_diario = ["📋 Pasar lista"]

        # Dibujamos el radio con una clave única que depende del rol para forzar el refresco
        diario = st.radio(
            " ",
            opciones_diario,
    ...
    # El resto de grupos (Gestión, Informes...) se mantienen con tu "if rol == 'admin':"
    # que ya los oculta correctamente.
...
if diario == "📋 Pasar lista":
    if rol == "cocina":
        st.info("💡 Tu perfil está configurado para consultar el **Panel de Cocina**. Selecciona esa opción en el menú lateral.")
    else:
        st.cache_data.clear() 
...
# ---------------------------------------------------------
# PANEL DE COCINA (Accesible para Admin y Cocina)
# ---------------------------------------------------------
