...
if rol == "admin":
    ...
    if rol == "cocina" and st.session_state.informes == "📊 Informe de situación en mesa":
        ...
        # Agrupar por curso
        conteo = resumen.groupby("nombre_curso").size().reset_index(name="Alumnos")

        st.subheader("Alumnos por curso")
        st.table(conteo.rename(columns={"nombre_curso": "Curso"}))

        st.subheader("Listado completo")
        st.table(resumen[["nombre_alumno", "nombre_curso"]].rename(columns={
