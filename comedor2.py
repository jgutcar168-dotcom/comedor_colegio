...
# ---------------------------------------------------------
# PASAR LISTA (ORDENADO ALFABÉTICAMENTE)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    ...
    if df_asistencia.empty or "fecha" not in df_asistencia.columns:
        import pandas as pd
        df_asistencia = pd.DataFrame(columns=["id", "fecha", "alumno_id", "curso_id", "asiste", "motivo"])

    fecha_hoy = datetime.now().strftime("%Y-%m-%d")
    
    if "fecha" not in df_asistencia.columns:
        st.error(f"Columnas detectadas en la tabla: {list(df_asistencia.columns)}")
        st.stop()
    ...
    if not cursos_disponibles:
        ...
        # Filtramos y ORDENAMOS por nombre antes de mostrar la lista
        alumnos_curso = df_alumnos[df_alumnos["curso_id"] == curso_id].copy()
        alumnos_curso = alumnos_curso.sort_values(by="nombre") 
        ...
        # Al estar 'alumnos_curso' ordenado, los checkboxes saldrán en orden A-Z
        for _, alumno in alumnos_curso.iterrows():
            registro_previo = asistencia_hoy[asistencia_hoy["alumno_id"] == alumno["id"]]
            valor_inicial = True if registro_previo.empty else bool(registro_previo["asiste"].iloc[0])

            estado_asiste = st.checkbox(
                alumno["nombre"],
                value=valor_inicial,
                key=f"asiste_{alumno['id']}_{curso_id}"
            )
        ...
        if st.button("💾 Guardar Asistencia", type="primary", use_container_width=True):
            registros = []
            for alu_id, asiste_val in estado.items():
                registros.append({
                    "fecha": fecha_hoy,
                    "alumno_id": int(alu_id),
                    "curso_id": int(curso_id),
                    "asiste": bool(asiste_val),
                    "motivo": ""
                })

            if registros:
                try:
                    res = supabase.table("asistencia").upsert(registros, on_conflict="alumno_id,fecha").execute()
                    if res.data:
                        st.success(f"✅ ¡CONSEGUIDO! {len(res.data)} registros grabados.")
                        st.cache_data.clear()
                        st.balloons()
                    else:
                        st.error("❌ El servidor no guardó nada.")
                except Exception as e:
...
# ---------------------------------------------------------
    ...
    df_hoy = df_asistencia[(df_asistencia["fecha"] == fecha_hoy) & (df_asistencia["asiste"] == True)]

    if df_hoy.empty:
        st.warning("⚠️ No hay datos de asistencia para hoy en la base de datos.")
    else:
        try:
            # 2. Asegurar IDs numéricos
            df_hoy["alumno_id"] = df_hoy["alumno_id"].astype(int)
            df_alumnos["id"] = df_alumnos["id"].astype(int)
            df_cursos["id"] = df_cursos["id"].astype(int)

            # 3. Merges paso a paso
            # Primero asistencia con alumnos para traer el nombre del niño y su curso_id
            resumen = df_hoy.merge(df_alumnos, left_on="alumno_id", right_on="id")
            
            # Segundo con cursos para traer el nombre del curso
            # Usamos curso_id_y que suele ser el del alumno tras el merge
            c_id = "curso_id_y" if "curso_id_y" in resumen.columns else "curso_id"
            resumen = resumen.merge(df_cursos, left_on=c_id, right_on="id")

            st.metric("TOTAL COMENSALES", len(resumen))
            
            # Agrupar por el nombre del curso (columna 'nombre_y' o 'nombre_curso')
            col_nombre_curso = "nombre_y" if "nombre_y" in resumen.columns else "nombre"
            conteo = resumen.groupby(col_nombre_curso).size().reset_index(name="Alumnos")
            st.table(conteo.rename(columns={col_nombre_curso: "Curso"}))
            
        except Exception as e:
            st.error(f"Error procesando los datos: {e}")
            # Muestra las columnas para saber qué falló
            # st.write("Columnas detectadas:", resumen.columns.tolist())


# ---------------------------------------------------------
# CONTROL DE ASISTENCIA (CORREGIDO - EXCLUYE "NINGUNO")
# ---------------------------------------------------------
    ...
    df_asistencia = db_select("asistencia")
    df_cursos_raw = db_select("cursos") # Cargamos todos los cursos
    fecha_hoy = datetime.now().strftime("%Y-%m-%d")

    # --- FILTRO CRÍTICO ---
    # Excluimos el curso "Ninguno" para que no cuente como pendiente
    df_cursos = df_cursos_raw[df_cursos_raw["nombre"].str.lower() != "ninguno"].copy()

    # 1. Obtenemos los cursos que YA han pasado lista
    if not df_asistencia.empty and "fecha" in df_asistencia.columns:
        # Filtramos registros de hoy
        df_hoy = df_asistencia[df_asistencia["fecha"] == fecha_hoy]
        # Obtenemos los IDs únicos de los cursos que han enviado datos hoy
        cursos_con_lista = df_hoy["curso_id"].unique()
    else:
        cursos_con_lista = []

    # 2. Calculamos los cursos que FALTAN
    df_cursos["id"] = df_cursos["id"].astype(int)
    
    # Cursos que han pasado lista hoy (dentro de los cursos válidos)
    df_ok = df_cursos[df_cursos["id"].isin(cursos_con_lista)]
    
    # Cursos que NO están en la lista de hoy (excluyendo "Ninguno")
    ...
    if df_ok.empty:
        ...
        # Mostramos la tabla sin índices y con nombre de columna limpio
        st.table(df_ok[["nombre"]].rename(columns={"nombre": "Curso"}))

    st.subheader("Cursos que FALTAN por pasar lista")
    if cursos_faltan.empty:
        st.success("¡Excelente! Todos los cursos operativos han pasado lista hoy.")
    else:
        st.error(f"Hay {len(cursos_faltan)} cursos pendientes.")
...
if rol == "admin":
   ...
   # ---------------------------------------------------------
    ...
    if rol == "cocina" and st.session_state.informes == "📊 Informe de situación en mesa":
        ...
        # Filtrar solo los que asisten hoy
        ...
        resumen = df_hoy.merge(df_alumnos, left_on="alumno_id", right_on="id")
        resumen = resumen.merge(df_cursos, left_on="curso_id", right_on="id", suffixes=("_alumno", "_curso"))

        st.metric("TOTAL COMENSALES", len(resumen))

        # Ojo al nombre de la columna: debe existir 'nombre_curso' tras el merge
        if "nombre_curso" in resumen.columns:
            col_curso = "nombre_curso"
        else:
            # por si el merge la deja como 'nombre' del curso
            col_curso = "nombre_curso" if "nombre_curso" in resumen.columns else "nombre"

        conteo = resumen.groupby(col_curso).size().reset_index(name="Alumnos")

        st.subheader("Alumnos por curso")
        st.table(conteo.rename(columns={col_curso: "Curso"}))

        # Nombres de alumno y curso para el listado
        col_alumno = "nombre_alumno" if "nombre_alumno" in resumen.columns else "nombre"
        st.subheader("Listado completo")
        st.table(resumen[[col_alumno, col_curso]].rename(columns={
            col_alumno: "Alumno",
            col_curso: "Curso"
        }))
    ...
    # (a partir de aquí sigues con TODO tu código de PDF tal como lo tenías)
        ...
        with c_btn2:
            ...
            if st.button("🖨️ Generar PDF de Situación", type="primary", use_container_width=True):
                ...
                if not any([m1_sel, m2_sel, m3_sel]):
                    ...
                    buffer = io.BytesIO()
                    # Configuración del documento (Apaisado y márgenes)
                    ...
                    # Encabezados personalizados según tu código previo
                    ...
                    # Cuerpo de la tabla con colores
                    for i in range(max_filas):
                        ...
                        for col_idx, lista in enumerate([m1_pdf, m2_pdf, m3_pdf]):
                            if i < len(lista):
                                ...
                                # El color viene de tu función externa obtener_info_etapa
                    ...
                    # --- FILA DE SUB-TOTALES ---
                    t1, t2, t3 = len(m1_pdf), len(m2_pdf), len(m3_pdf)
                    tabla_data.append([f"Total: {t1}", f"Total: {t2}", f"Total: {t3}"])
                    ...
                    estilos_tabla.append(("FONTSIZE", (0, idx_last), (-1, idx_last), 11))

                    # Generar tabla (ancho automático repartido en 3)
                    ancho_util = landscape(A4)[0] - 60 # Restamos márgenes
                    t_mesas = Table(tabla_data, colWidths=[ancho_util/3]*3, repeatRows=1)
                    t_mesas.setStyle(TableStyle(estilos_tabla))
                    elementos.append(t_mesas)
        
                    # --- TOTAL FINAL DESTACADO ---
                    elementos.append(Spacer(1, 20))
                    total_total = t1 + t2 + t3
                    estilo_total = f"""
                        <para align='right' size='14'>
                        <b>TOTAL COMENSALES HOY: <font color='blue'>{total_total}</font></b>
                        </para>
                    """
                    elementos.append(Paragraph(estilo_total, estilos_p['Normal']))
                    ...
                    def add_page_number(canvas, doc):
                        ...
                        canvas.drawRightString(landscape(A4)[0] - 30, 20, text)
        ...
        # =========================================================
        # INFORME DE FALTAS (UNA HOJA POR CURSO CON TOTALES)
        # =========================================================
        ...
        if st.button("Generar PDF de Faltas"):
            buffer = io.BytesIO()
            # Usamos landscape para tener espacio para los 31 días
            c = canvas.Canvas(buffer, pagesize=landscape(A4))
            page_width, page_height = landscape(A4)
            ...
            for i, curso in enumerate(cursos_a_procesar):
                # Si no es el primer curso, saltamos de página
                if i > 0:
                    add_page_number(c)
                    c.showPage()

                # Reiniciar posición y dibujar cabecera de página
                y_offset = page_height - 170
                draw_logo_centered(c, page_width, page_height - 200)
        
                c.setFont("Helvetica-Bold", 16)
                c.drawCentredString(page_width/2, page_height - 140, f"Informe Mensual de Faltas - {mes_f}/{año_f}")
        
                c.setFont("Helvetica-Bold", 14)
                c.drawString(30, y_offset, f"Curso: {curso['nombre']}")
                y_offset -= 25

                # Preparar datos de la tabla (Cabecera)
                tabla_data = [["Alumno"] + [str(d) for d in range(1, dias_mes+1)] + ["Total"]]
                alumnos_curso = df_alumnos[df_alumnos["curso_id"] == curso["id"]].sort_values("nombre")
                ...
                for fila_idx, (_, alumno) in enumerate(alumnos_curso.iterrows()):
                    ...
                    total_faltas_clase += faltas_alumno
                    tabla_data.append(fila)
            
                    # Sombreado alternado
                    if (fila_idx + 1) % 2 == 0:
                        estilos_celdas.append(("BACKGROUND", (0, fila_idx + 1), (-1, fila_idx + 1), colors.whitesmoke))

                # Añadir Fila de Totales del Curso
                fila_total = ["TOTAL FALTAS DEL CURSO"] + [""] * dias_mes + [total_faltas_clase]
                tabla_data.append(fila_total)
                idx_total = len(tabla_data) - 1
        
                # Estilo para la fila de totales
                estilos_celdas.append(("BACKGROUND", (0, idx_total), (-1, idx_total), colors.lightgrey))
                estilos_celdas.append(("FONTNAME", (0, idx_total), (-1, idx_total), "Helvetica-Bold"))
                estilos_celdas.append(("SPAN", (0, idx_total), (dias_mes, idx_total)))
                estilos_celdas.append(("ALIGN", (0, idx_total), (0, idx_total), "RIGHT"))

                # Crear tabla y dibujar
                # Ancho nombre: 135, Días: 18 cada uno, Total: 25
                tabla = Table(tabla_data, colWidths=[135] + [18]*dias_mes + [25])
                tabla.setStyle(TableStyle(estilos_celdas))

                w, h = tabla.wrap(0, 0)
                # Dibujamos la tabla (si es muy larga, ReportLab canvas no hace salto automático, 
                # pero para una lista de clase normal en A4 apaisado cabe perfectamente)
            ...
            st.download_button(
                ...
                data=buffer.getvalue(),
                file_name=f"informe_faltas_{mes_f}_{año_f}.pdf",
                mime="application/pdf",
                use_container_width=True
            )
