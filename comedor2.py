...
# ---------------------------------------------------------
# PASAR LISTA (VERSIÓN CON DEBUG DE GUARDADO)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    ...
    if not cursos_disponibles:
        ...
        curso_sel = st.selectbox("Selecciona curso", cursos_disponibles, format_func=lambda x: x["nombre"])
        curso_id = int(curso_sel["id"])

        alumnos_curso = df_alumnos[df_alumnos["curso_id"] == curso_id]
        asistencia_hoy = df_asistencia[(df_asistencia["fecha"] == fecha_hoy) & (df_asistencia["curso_id"] == curso_id)]

        estado = {}
        
        # Separación visual
        alumnos_comedor = alumnos_curso[alumnos_curso["solo_pt_al"].fillna(False) == False]
        ...
        for _, alumno in alumnos_comedor.iterrows():
            reg = asistencia_hoy[asistencia_hoy["alumno_id"] == alumno["id"]]
            val = True if reg.empty else bool(reg["asiste"].iloc[0])
            estado[alumno["id"]] = st.checkbox(alumno["nombre"], value=val, key=f"as_{alumno['id']}")

        if not alumnos_solo_pt.empty:
            st.divider()
            st.subheader("📘 Solo Apoyo")
            for _, alumno in alumnos_solo_pt.iterrows():
                reg = asistencia_hoy[asistencia_hoy["alumno_id"] == alumno["id"]]
                val = True if reg.empty else bool(reg["asiste"].iloc[0])
                estado[alumno["id"]] = st.checkbox(f"**{alumno['nombre']}**", value=val, key=f"pt_{alumno['id']}")
        ...
        if st.button("💾 Guardar Asistencia", type="primary", use_container_width=True):
            ...
            if registros:
                # --- PRUEBA DE FUEGO: Intento de guardado con captura de error ---
                try:
                    resultado = db_upsert("asistencia", registros, conflict_cols="alumno_id,fecha")
                    
                    # Verificamos si realmente se guardó algo
                    st.success(f"✅ Proceso finalizado para {curso_sel['nombre']}")
                    st.balloons()
                    st.cache_data.clear() # Limpiamos cache para que el admin vea los cambios
                    
                except Exception as e:
                    st.error("❌ ERROR CRÍTICO AL GUARDAR EN BASE DE DATOS")
                    st.info(f"Detalle del error: {e}")
            else:
                st.error("No hay alumnos en la lista.")
# ---------------------------------------------------------
