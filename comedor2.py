        ...
        with c_btn2:
            if st.button("🖨️ Generar PDF de Situación", type="primary", use_container_width=True):
                if not any([m1_sel, m2_sel, m3_sel]):
                    ...
                    c.setFont("Helvetica-Bold", 18)
                    c.drawCentredString(page_width/2, page_height - 80, "INFORME DE SITUACIÓN EN MESA")
                    c.setFont("Helvetica", 12)
                    c.drawCentredString(page_width/2, page_height - 100, f"Generado el {datetime.now().strftime('%d/%m/%Y %H:%M')}")

                    # Preparar datos ordenados para el PDF
                    m1_pdf = sorted(m1_sel, key=lambda x: x['curso_id'])
                    m2_pdf = sorted(m2_sel, key=lambda x: x['curso_id'])
                    m3_pdf = sorted(m3_sel, key=lambda x: x['curso_id'])
                    ...
                    c.save()
                    st.download_button("📩 Descargar PDF de Situación", buffer.getvalue(), 
                                     f"situacion_mesas_{datetime.now().strftime('%Y%m%d')}.pdf", 
