    ...
    if not cursos_disponibles:
        ...
        if alumnos_solo_pt.empty:
            ...
            for _, alumno in alumnos_solo_pt.iterrows():
                registro_previo = asistencia_hoy[asistencia_hoy["alumno_id"] == alumno["id"]]
                # Para estos alumnos, si es nuevo registro, ponemos False por defecto para que el tutor lo marque activamente
                valor_inicial = False if registro_previo.empty else bool(registro_previo["asiste"].iloc[0])
                
                # Checkbox con estilo distinto
                estado[alumno["id"]] = st.checkbox(
        ...
        if st.button("Guardar asistencia", type="primary", use_container_width=True):
            ...
            if registros:
                ...
                st.error("No hay alumnos para registrar.")
