...
# ---------------------------------------------------------
# PASAR LISTA (VERSIÓN CORREGIDA Y ROBUSTA)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    ...
    # --- RED DE SEGURIDAD PARA EL DATAFRAME ---
    if not res_asist.data:
        # Si no hay datos hoy, creamos el DF con las columnas necesarias para evitar el KeyError
        df_asistencia = pd.DataFrame(columns=["id", "fecha", "alumno_id", "curso_id", "asiste", "motivo"])
    else:
        df_asistencia = pd.DataFrame(res_asist.data)

    # 3. Determinar cursos según rol
    if rol == "admin":
        cursos_disponibles = df_cursos.to_dict(orient="records")
    else:
        # Aseguramos que prof["curso_id"] existe y lo procesamos
    ...
    if not cursos_disponibles:
        ...
        # Filtrar asistencia ya existente para este curso
        asist_actual = df_asistencia[df_asistencia["curso_id"] == c_id]

        # 4. Generar el formulario de checks
        estado_checks = {}
        
        # Convertimos a diccionario para búsqueda ultra rápida y evitar errores de indexación
        # dict_asistencia = { alumno_id: valor_asiste }
        dict_asistencia = {str(row["alumno_id"]): row["asiste"] for _, row in asist_actual.iterrows()}

        for _, alu in alumnos_curso.iterrows():
            id_alu_str = str(alu["id"])
            
            # Si el alumno ya tiene registro hoy, usamos ese valor, si no, True (asiste por defecto)
            valor_defecto = dict_asistencia.get(id_alu_str, True)

            # Checkbox para el profesor
            estado_checks[alu["id"]] = st.checkbox(
                alu["nombre"], 
                value=bool(valor_defecto), 
                key=f"v4_{c_id}_{alu['id']}"
            )
        ...
        if st.button("💾 GUARDAR AHORA", type="primary"):
            ...
            try:
                # Borramos registros previos de este curso hoy para evitar duplicados
    ...
    if st.session_state.informes == "📝 Informes PDF":
        ...
        if st.button("Generar PDF de Faltas", type="primary"):
            ...
            for i, curso in enumerate(cursos_a_procesar):
                ...
                anchos_dias = (page_width - 180) / dias_mes
                tabla = Table(tabla_data, colWidths=[140] + [anchos_dias]*dias_mes + [30])
                tabla.setStyle(TableStyle(estilos))
                
                w, h = tabla.wrap(0, 0)
                tabla.drawOn(c, 25, page_height - 160 - h)
                add_page_number(c)
