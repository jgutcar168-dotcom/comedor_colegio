    ...
    if st.session_state.informes == "📝 Informes PDF":
        ...
        with c_btn2:
            ...
            if st.button("🖨️ Generar PDF de Situación", type="primary", use_container_width=True):
                ...
                # Lista de IDs de alumnos que asisten hoy
                ...
                elif not ids_asisten_hoy:
                    st.warning("No hay registros de asistencia para el día de hoy. Se generará con todos los seleccionados.")
                # Opcional: podrías detener la ejecución aquí o seguir
                    ids_asisten_hoy = [a["id"] for a in (m1_sel + m2_sel + m3_sel)] 

                # 2. Filtrar las listas seleccionadas: solo los que están en ids_asisten_hoy
                m1_final = [a for a in m1_sel if a["id"] in ids_asisten_hoy]
                m2_final = [a for a in m2_sel if a["id"] in ids_asisten_hoy]
                m3_final = [a for a in m3_sel if a["id"] in ids_asisten_hoy]

                # --- A partir de aquí, el código del PDF usa m1_final, m2_final y m3_final ---
                buffer = io.BytesIO()
                ...
                c.setFont("Helvetica-Bold", 18)
                c.drawCentredString(page_width/2, page_height - 80, "INFORME DE SITUACIÓN EN MESA (ASISTENCIA REAL)")
                c.setFont("Helvetica", 12)
                c.drawCentredString(page_width/2, page_height - 100, f"Día: {datetime.now().strftime('%d/%m/%Y')} | Alumnos presentes hoy")

                # Ordenar por curso para el PDF
                m1_pdf = sorted(m1_final, key=lambda x: x['curso_id'])
                m2_pdf = sorted(m2_final, key=lambda x: x['curso_id'])
                m3_pdf = sorted(m3_final, key=lambda x: x['curso_id'])
    
                max_filas = max(len(m1_pdf), len(m2_pdf), len(m3_pdf))
                tabla_data = [["FILA IZQUIERDA", "FILA CENTRO", "FILA DERECHA"]]
    
                # ... (Resto del código de estilos y celdas igual que el anterior)
                estilos = [
                    ...
                    ("FONTSIZE", (0,0), (-1,-1), 10), # Bajamos un punto el tamaño por si hay muchos
                ]
                ...
                for i in range(max_filas):
                    ...
                    for col_idx, lista in enumerate([m1_pdf, m2_pdf, m3_pdf]):
                        if i < len(lista):
                            ...
                            estilos.append(("BACKGROUND", (col_idx, i+1), (col_idx, i+1), color_f))
                        else:
                            fila_cont.append("")
                    tabla_data.append(fila_cont)

                t_mesas = Table(tabla_data, colWidths=[(page_width-100)/3]*3)
                t_mesas.setStyle(TableStyle(estilos))
                w_t, h_t = t_mesas.wrap(page_width, page_height)
                t_mesas.drawOn(c, 50, (page_height - 130) - h_t)
    
                c.save()
                st.download_button("📩 Descargar PDF de Situación Real", buffer.getvalue(), 
                                f"mesas_asistencia_{fecha_hoy_str}.pdf", 
