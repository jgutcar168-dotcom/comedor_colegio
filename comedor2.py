...
# ---------------------------------------------------------
# PASAR LISTA (VERSIÓN FUERZA BRUTA - BORRADO Y CARGA)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    ...
    if not cursos_disponibles:
        ...
        curso_sel = st.selectbox(
            ...
            key="sel_curso_final_v3"
        )
        
        c_id = int(curso_sel["id"])

        # 2. Carga de alumnos y asistencia actual
        alumnos_curso = df_alumnos[df_alumnos["curso_id"] == c_id].copy().sort_values(by="nombre")
        asist_actual = df_asistencia[(df_asistencia["fecha"] == fecha_hoy) & (df_asistencia["curso_id"] == c_id)]

        # 3. Formulario de Checkboxes
        # IMPORTANTE: No usamos st.form para evitar que Streamlit bloquee el estado
        st.subheader(f"Lista de {curso_sel['nombre']}")
        
        estado_checks = {}
        for _, alu in alumnos_curso.iterrows():
            # Buscamos el valor real en la base de datos
            reg = asist_actual[asist_actual["alumno_id"] == alu["id"]]
            
            # Si no hay registro previo, marcamos True. Si lo hay, lo que diga la DB.
            valor_inicial = True if reg.empty else bool(reg["asiste"].iloc[0])

            # Key hiper-específica para evitar caché mágica de Streamlit
            estado_checks[alu["id"]] = st.checkbox(
                alu["nombre"],
                value=valor_inicial,
                key=f"v3_{c_id}_{alu['id']}_{len(asist_actual)}" 
            )

        # 4. BOTÓN DE GUARDADO TOTAL
        if st.button("💾 GUARDAR CAMBIOS", type="primary", use_container_width=True):
            registros = []
            for a_id, asiste in estado_checks.items():
                registros.append({
                    "fecha": fecha_hoy,
                    "alumno_id": int(a_id),
                    "curso_id": int(c_id),
            ...
            try:
                # PASO 1: Borramos lo que hubiera hoy para este curso (Limpieza total)
                # Esto es más seguro que el upsert si hay problemas de claves
                supabase.table("asistencia").delete().eq("fecha", fecha_hoy).eq("curso_id", c_id).execute()
                
                # PASO 2: Insertamos los nuevos datos
                res = supabase.table("asistencia").insert(registros).execute()
                
                if res.data:
                    st.success(f"✅ ¡Actualizado! {len(res.data)} alumnos procesados.")
                    st.cache_data.clear()
                    # Borramos df_asistencia de la sesión para obligar a recargar
                    if "df_asistencia" in st.session_state:
                        del st.session_state["df_asistencia"]
                    
                    st.balloons()
                    # Esperamos un segundo y recargamos
                    st.rerun()
                else:
                    st.error("El servidor no devolvió confirmación de guardado.")
            except Exception as e:
