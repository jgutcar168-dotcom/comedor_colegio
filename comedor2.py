...
# --- PROTECCIÓN DE SEGURIDAD PARA EL ROL COCINA ---
# Si el rol es cocina, forzamos que la variable 'diario' sea algo válido para ellos
if rol == "cocina":
    if st.session_state.get("diario") not in ["🍽️ Panel cocina", "📊 Situación en mesa"]:
...
with st.sidebar:
    ...
    with st.expander("📅 Diario", expanded=True):
        ...
        if rol == "admin":
            ...
            # Admin usa la key estándar
            key_menu = "diario"
        elif rol == "cocina":
            opciones_diario = ["🍽️ Panel cocina", "📊 Situación en mesa"]
            # Cocina usa una key NUEVA para forzar que desaparezca "Pasar lista"
            key_menu = "menu_cocina"
        else:
        ...
        diario = st.radio(
            ...
            key=key_menu, 
            on_change=set_nav,
            args=(st.session_state.get(key_menu), "diario")
        )

        # Sincronizamos la selección para que el resto de la app sepa qué mostrar
