...
# ---------------------------------------------------------
    if st.session_state.informes == "📝 Informes PDF":
        ...
        # =========================
        # INFORME DIARIO PARA COCINA (REVISADO Y ASEGURADO)
        # =========================
        ...
        if st.button("Generar PDF Diario", type="primary"):
            # 1. Aseguramos tipos de datos antes de filtrar (Evita que la tabla salga vacía)
            df_asistencia["alumno_id"] = pd.to_numeric(df_asistencia["alumno_id"], errors='coerce')
            df_alumnos["id"] = pd.to_numeric(df_alumnos["id"], errors='coerce')
            df_alumnos["curso_id"] = pd.to_numeric(df_alumnos["curso_id"], errors='coerce')
            df_cursos["id"] = pd.to_numeric(df_cursos["id"], errors='coerce')
            ...
            if comen.empty:
                ...
                # 3. Merges Limpios
                # Primero unimos con alumnos para saber quién es quién
                resumen = comen.merge(df_alumnos[["id", "nombre", "curso_id"]], left_on="alumno_id", right_on="id")
                
                # Unimos con cursos para tener el nombre del curso (usamos el curso_id del alumno)
                resumen = resumen.merge(df_cursos[["id", "nombre"]], left_on="curso_id", right_on="id", suffixes=("", "_curso"))

                # 4. Preparación de la Tabla
                tabla_data = [["Curso", "Comensales"]]
                conteo = resumen.groupby("nombre_curso").size().reset_index(name="Cantidad")
                
                # Ordenar por curso para que el PDF sea profesional
                ...
                # 6. Observaciones (Motivos)
                y_actual -= 40
                c.setFont("Helvetica-Bold", 14)
                c.drawString(50, y_actual, "Observaciones / Motivos:")
                y_actual -= 20

                # Filtrar alumnos con motivo (asistan o no, para que cocina lo sepa)
                obs = df_dia[df_dia["motivo"].fillna("") != ""].merge(df_alumnos, left_on="alumno_id", right_on="id")
                ...
                if obs.empty:
                    ...
                    for _, row in obs.iterrows():
                        if y_actual < 80: # Control de salto de página simple
                            add_page_number(c)
                            c.showPage()
                            y_actual = 800
