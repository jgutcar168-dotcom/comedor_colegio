...
if rol == "admin":
    ...
    # ---------------------------------------------------------
        ...
        # Unimos con alumnos para tener los nombres
        # Usamos inner join para asegurar que solo vemos asistencias con alumno válido
        ...
        # Verificamos qué columnas existen realmente para evitar el KeyError
        columnas_disponibles = datos.columns.tolist()
        
        # Lista de columnas que queremos mostrar (ajustada a tu nueva DB)
        columnas_a_mostrar = ["alumno_id", "nombre", "asiste", "motivo", "curso_id"]
        
        # Solo añadimos curso_academico si existe en el DataFrame
        ...
        if st.button("Guardar cambios"):
            try:
                ...
                for _, row in editable.iterrows():
                    ...
                    # Añadimos curso_academico solo si estaba en el editor
                    if "curso_academico" in row:
                        reg["curso_academico"] = row["curso_academico"]
                    
                    registros_update.append(reg)

                # Usamos la conexión directa que sabemos que funciona bien
        ...
        if st.button("Eliminar registro seleccionado", type="secondary"):
            ...
            try:
                supabase.table("asistencia").delete().eq("alumno_id", alumno_id_borrar).eq("fecha", fecha_str).execute()
                st.success("Registro eliminado. Recargando...")
                st.cache_data.clear()
                st.rerun()
            except Exception as e:
