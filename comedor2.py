...
# ---------------------------------------------------------
# PASAR LISTA (ACTUALIZADO: COMEDOR + APOYO PT/AL)
# ---------------------------------------------------------
if diario == "📋 Pasar lista":
    ...
    if not cursos_disponibles:
        ...
        # SECCIÓN 1: COMEDOR
        st.markdown("#### 🍽️ Alumnos de Comedor")
        if alumnos_comedor.empty:
            st.info("No hay alumnos de comedor en este curso.")
        else:
            for _, alumno in alumnos_comedor.iterrows():
        ...
        # SECCIÓN 2: SOLO APOYO (PT/AL)
        st.markdown("#### 📘 Control Presencia PT/AL (No Comedor)")
        if alumnos_solo_pt.empty:
            st.caption("No hay alumnos de 'Solo Apoyo' registrados en este curso.")
        else:
            st.info("Marque si el alumno ha asistido al centro hoy para informar al equipo de especialistas.")
            for _, alumno in alumnos_solo_pt.iterrows():
                # Buscamos si ya existe un registro de hoy en la base de datos
                registro_previo = asistencia_hoy[asistencia_hoy["alumno_id"] == alumno["id"]]
        
                # --- CAMBIO AQUÍ: valor_inicial ahora es True por defecto ---
                if registro_previo.empty:
                    valor_inicial = True  # El alumno "está" a menos que el profe diga lo contrario
                else:
                    valor_inicial = bool(registro_previo["asiste"].iloc[0])
        
                # Checkbox con estilo distinto para diferenciarlo de comedor
                estado[alumno["id"]] = st.checkbox(
                    f"**{alumno['nombre']}** (Presencia para especialista)",
                    value=valor_inicial,
                    key=f"asiste_pt_{alumno['id']}_{curso_id}",
                    help="Este alumno no cuenta para cocina, solo para control de PT/AL"
                )
        ...
        if st.button("💾 Guardar Asistencia", type="primary", use_container_width=True):
            ...
            if registros:
                try:
                    ...
                    st.balloons()
                    # No hacemos rerun inmediato para que vean el éxito, o puedes ponerlo si prefieres limpiar
                except Exception as e:
                    st.error(f"Error al guardar: {e}")
            else:
                st.error("No hay datos de alumnos para guardar.")
# ---------------------------------------------------------
# PANEL DE COCINA (CORREGIDO Y BLINDADO)
# ---------------------------------------------------------
elif diario == "🍽️ Panel cocina":
    st.cache_data.clear() # Limpieza forzada para ver datos reales
    ...
    # 1. Filtro por fecha
    df_hoy = df_asistencia[df_asistencia["fecha"] == fecha_hoy].copy()
    
    if df_hoy.empty:
        st.warning("Aún no se ha pasado lista hoy en ningún curso.")
        st.stop()

    # 2. Asegurar tipos de datos para el merge (Evita que desaparezcan registros)
    df_hoy["alumno_id"] = df_hoy["alumno_id"].astype(int)
    df_alumnos["id"] = df_alumnos["id"].astype(int)

    # 3. Cruce Asistencia + Alumnos
    resumen = df_hoy.merge(df_alumnos, left_on="alumno_id", right_on="id")

    # 4. FILTRO CRÍTICO: Solo comensales (Asiste=True y NO es solo_pt_al)
    # Usamos fillna(False) por si hay nulos en la nueva columna
    comen_real = resumen[
        (resumen["asiste"] == True) & 
        (resumen["solo_pt_al"].fillna(False) == False)
    ].copy()

    if comen_real.empty:
        st.info("Hay registros de hoy, pero ningún alumno de los presentes figura como comensal.")
    else:
        # Identificar columna de curso correctamente
        col_curso = "curso_id_y" if "curso_id_y" in comen_real.columns else "curso_id_x" if "curso_id_x" in comen_real.columns else "curso_id"
        comen_real["curso_id_final"] = comen_real[col_curso].astype(int)
        df_cursos["id"] = df_cursos["id"].astype(int)

        # 5. Cruce final con Cursos para obtener nombres
        comen_real = comen_real.merge(df_cursos, left_on="curso_id_final", right_on="id", suffixes=("", "_curso"))

        st.metric("TOTAL COMENSALES", len(comen_real))

        col1, col2 = st.columns(2)
        with col1:
            st.write("### Comensales por Curso")
            conteo = comen_real.groupby("nombre_curso").size().reset_index(name="Cantidad")
            st.table(conteo)

        with col2:
            st.write("### Observaciones Importantes")
            obs = comen_real[comen_real["motivo"].fillna("") != ""][["nombre", "motivo"]]
            if not obs.empty:
                st.dataframe(obs, hide_index=True)
            else:
                st.write("No hay observaciones.")

        st.write("### Lista Completa de Comensales")
        lista = comen_real[["nombre", "nombre_curso"]].rename(columns={"nombre": "Alumno", "nombre_curso": "Curso"})
        st.dataframe(lista, hide_index=True)


# ---------------------------------------------------------
# CONTROL DE ASISTENCIA (CORREGIDO Y BLINDADO)
# ---------------------------------------------------------
    ...
    # Filtramos todos los registros de hoy para saber qué profes han guardado
    df_hoy = df_asistencia[df_asistencia["fecha"] == fecha_hoy].copy()

    # Aseguramos que el ID del curso sea tratado igual en ambas tablas
    df_hoy["curso_id"] = df_hoy["curso_id"].astype(int)
    df_cursos["id"] = df_cursos["id"].astype(int)

    # Obtenemos los IDs únicos de cursos que ya tienen datos en la tabla asistencia
    ...
    if cursos_faltan.empty:
        st.success("¡Enhorabuena! Todos los cursos han pasado lista hoy.")
    else:
        st.error(f"Faltan {len(cursos_faltan)} cursos por pasar lista.")
