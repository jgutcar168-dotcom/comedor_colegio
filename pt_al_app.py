...
# --- PANTALLA 2: GESTIÓN DE ALUMNOS (ASIGNAR PT/AL Y TIPO) ---
elif opcion == "👥 Gestión de Alumnos":
    st.title("Configuración de Alumnos PT/AL")
    st.info("Utilice esta pantalla para dar de alta alumnos en su programa y especificar si se quedan al comedor o solo vienen a apoyo.")
    ...
    if not df_alumnos.empty:
        ...
        for _, fila in df_merge.sort_values(["nombre_y", "nombre_x"]).iterrows():
            ...
            with c2:
                # Checkbox para saber si pertenece al programa
                check_pt = st.checkbox("Incluir", value=fila['es_pt_al'], key=f"pt_{fila['id_x']}")
                if check_pt != fila['es_pt_al']:
                    supabase.table("alumnos").update({"es_pt_al": check_pt}).eq("id", fila['id_x']).execute()
                    st.rerun()

            with c3:
                # Checkbox para saber si NO come en el centro (solo si es PT/AL)
                if check_pt:
                    check_solo = st.checkbox("Solo Apoyo", value=fila['solo_pt_al'], key=f"solo_{fila['id_x']}")
                    if check_solo != fila['solo_pt_al']:
                        supabase.table("alumnos").update({"solo_pt_al": check_solo}).eq("id", fila['id_x']).execute()
                        st.rerun()
                else:
                    st.write("-")