...
if rol == "admin":
    ...
    # ---------------------------------------------------------
        ...
        if not df_profes.empty:
            prof_sel = st.selectbox("Selecciona profesor", df_profes.to_dict(orient="records"), format_func=lambda x: x["usuario"])
            nueva_pass = st.text_input("Nueva contraseña", type="password")

            if st.button("Actualizar contraseña"):
                db_upsert("profesores", [{
                    "id": prof_sel["id"],
                    "usuario": prof_sel["usuario"],
                    "password": nueva_pass,
                    "curso_id": prof_sel["curso_id"]
                }])
