...
if rol == "admin":
    ...
    # ---------------------------------------------------------
        ...
        if not df_profes.empty:
            ...
            with col_edit2:
                ...
                ids_actuales = str(prof_sel.get("curso_id", "")).split(",")
    ...
    if st.session_state.informes == "📝 Informes PDF":
       ...
       # =========================
        ...
        # =========================================================
        # CUADRANTE POR RANGO DE FECHAS (MAESTROS ACTIVOS)
        # =========================================================
        st.subheader("Cuadrante de Maestros (Personalizado)")

        # 1. Selector de Rango de Fechas
        rango_m = st.date_input(
            "Selecciona el intervalo de fechas para el informe",
            value=[datetime.now().date(), datetime.now().date()],
            key="rango_maestros_filter"
        )

        # Solo procedemos si el usuario ha seleccionado un inicio y un fin
        if isinstance(rango_m, (list, tuple)) and len(rango_m) == 2:
            ...
            if st.button("Generar Informe por Rango"):
                df_profes = db_select("profesores")
                df_comidas_raw = db_select("maestros_comidas")
                df_agua_raw = db_select("maestros_agua")

                # Crear lista de objetos fecha para las columnas de la tabla
                dias_rango = []
                curr = fecha_inicio
                while curr <= fecha_fin:
                    dias_rango.append(curr)
                    # CORRECCIÓN AQUÍ: Usamos la referencia completa por si acaso
                    curr += timedelta(days=1) 
        
                num_dias = len(dias_rango)

                # --- FILTRAR POR RANGO ---
                comidas_rango = df_comidas_raw[(df_comidas_raw["fecha"] >= f_ini_str) & (df_comidas_raw["fecha"] <= f_fin_str)]
                ids_comen = comidas_rango["maestro_id"].unique()
                ...
                ids_agua = agua_rango["maestro_id"].unique()
                ...
                c.setFont("Helvetica-Bold", 16)
                titulo = f"Control Maestros: {fecha_inicio.strftime('%d/%m/%y')} al {fecha_fin.strftime('%d/%m/%y')}"
                c.drawCentredString(page_width/2, page_height - 80, titulo)

                y_pos = page_height - 100

                # --- 1. TABLA DE COMIDAS ---
                c.setFont("Helvetica-Bold", 11)
                c.drawString(30, y_pos, "Maestros que han comido en este periodo:")
                y_pos -= 15

                if not profes_comen.empty:
                    data_c = [["Maestro"] + [str(d.day) for d in dias_rango] + ["Total"]]
            
                    for _, prof_row in profes_comen.iterrows():
                        fila = [prof_row["usuario"]]
                        total = 0
                        for d in dias_rango:
                            f_str = d.strftime("%Y-%m-%d")
                            asiste = not comidas_rango[(comidas_rango["maestro_id"] == prof_row["id"]) & (comidas_rango["fecha"] == f_str)].empty
                            fila.append("X" if asiste else "")
                            if asiste: total += 1
                        fila.append(total)
                        data_c.append(fila)

                    ancho_dia = 18 if num_dias > 15 else 25
                    t1 = Table(data_c, colWidths=[90] + [ancho_dia]*num_dias + [35])
                    t1.setStyle(TableStyle([
                        ("GRID", (0,0), (-1,-1), 0.5, colors.grey),
                        ("FONTSIZE", (0,0), (-1,-1), 7 if num_dias > 20 else 8),
                        ("BACKGROUND", (0,0), (-1,0), colors.lightgrey),
                    ...
                    c.drawString(40, y_pos, "No hay registros de comidas para este periodo.")
                    y_pos -= 30
                ...
                if not profes_agua.empty:
                    data_a = [["Maestro"] + [str(d.day) for d in dias_rango]]
                    for _, prof_row in profes_agua.iterrows():
                        fila = [prof_row["usuario"]]
                        for d in dias_rango:
                            f_str = d.strftime("%Y-%m-%d")
                            reg = agua_rango[(agua_rango["maestro_id"] == prof_row["id"]) & (agua_rango["fecha"] == f_str)]
                            if not reg.empty:
                    ...
                    t2 = Table(data_a, colWidths=[90] + [ancho_dia]*num_dias)
                    t2.setStyle(TableStyle([
                        ("GRID", (0,0), (-1,-1), 0.5, colors.grey),
                        ("FONTSIZE", (0,0), (-1,-1), 6 if num_dias > 20 else 7),
                        ("BACKGROUND", (0,0), (-1,0), colors.lightgrey),
                        ("ALIGN", (1,0), (-1,-1), "CENTER"),
                    ]))
                    w2, h2 = t2.wrap(0,0)
                    ...
                    c.setFont("Helvetica", 9)
                    c.drawString(40, y_pos, "No hay registros de agua para este periodo.")

                c.setFont("Helvetica-Oblique", 8)
                c.drawString(30, 20, "* Tabla de agua formato: (Botellas 0.25€ | Botellas 0.60€)")
        
                c.save()
                st.download_button(
                    "Descargar Informe Rango", 
                    data=buffer.getvalue(), 
                    file_name=f"informe_maestros_{f_ini_str}_al_{f_fin_str}.pdf"
                )
        # =========================================================
